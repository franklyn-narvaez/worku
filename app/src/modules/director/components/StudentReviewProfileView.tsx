...
export default function StudentReviewProfileView() {
	...
	const [hasViewedGrades, setHasViewedGrades] = useState(false);
	...
	const [_, setConfirmAction] = useState<'APPROVED' | 'REJECTED' | null>(null);
	...
	const fetchProfile = useCallback(async () => {
		...
		try {
			...
			const res = await fetch(`${REVIEW_PROFILE}/${id}/profile`, authOptions);
	...
	useEffect(() => {
		fetchProfile();
		const reviewed = localStorage.getItem(`reviewed_grades_${id}`);
		if (reviewed === 'true') {
			setHasViewedGrades(true);
		}
	}, [fetchProfile, id]);
	...
	const handleBack = () => {
		if (state?.from) {
			...
			navigate(DIRECTOR_REVIEW_PROFILES);
		}
	};

	const handleStatusChange = async (newStatus: 'APPROVED' | 'REJECTED', comment?: string) => {
		if (!id) return;

		if (newStatus === 'APPROVED' && !hasViewedGrades) {
			toast.warn('Debes revisar el certificado de notas antes de aprobar el perfil.');
			return;
		}

		try {
			const authOptions = await createAuthFetchOptions();
			const res = await fetch(`${REVIEW_PROFILE}/${id}/status`, {
				...authOptions,
				method: 'PATCH',
				headers: {
			...
			if (!res.ok) {
				const err = await res.json().catch(() => ({ message: 'Error desconocido' }));
				toast.error(err.error || err.message || 'Error al actualizar el estado');
				return;
			}
	...
	const openStatusModal = (newStatus: 'APPROVED' | 'REJECTED') => {
		if (!hasViewedGrades) {
			toast.warn('Debes revisar el certificado de notas antes de realizar esta acción.');
			return;
		}
		...
		if (newStatus === 'REJECTED') {
			...
			setConfirmMessage('¿Estás seguro de aprobar este perfil?');
				...
				<div className="flex items-center gap-2">
					{profile.Grades && (
						<a
							...
							onClick={() => {
								setHasViewedGrades(true);
								localStorage.setItem(`reviewed_grades_${id}`, 'true');
							}}
						>
							<Button
								variant={hasViewedGrades ? 'default' : 'outline'}
								size="sm"
			...
			<Dialog open={showRejectModal} onOpenChange={setShowRejectModal}>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						...
						<DialogDescription>Por favor, escribe un comentario indicando el motivo del rechazo.</DialogDescription>
					</DialogHeader>
					...
					<DialogFooter className="flex justify-end gap-2 pt-4">
						...
						<Button
							...
							onClick={() => handleStatusChange('REJECTED', rejectComment)}
						>
							Confirmar rechazo
						</Button>
					</DialogFooter>
