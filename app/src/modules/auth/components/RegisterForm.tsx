...
function RegisterForm(props: { colleges: College[] }) {
	...
	const onSubmit = handleSubmit(async (data) => {
		...
		if (res.status === 400)
			return toast.error("El usuario ya existe o hay un error en los datos.");
		if (res.status === 500)
			return toast.error("Error al registrar el usuario.");
		...
		<div className="h-full flex items-center justify-center">
			<FormProvider {...methods}>
				<form onSubmit={onSubmit} className="w-1/4">
					...
					<label
						htmlFor="collegeId"
						className="text-slate-900 mb-2 block text-sm"
					>
						Escuela
					</label>
					<select
					...
					{methods.formState.errors.collegeId && (
						<span className="text-red-500 text-sm">
							{methods.formState.errors.collegeId.message}
						</span>
					)}
