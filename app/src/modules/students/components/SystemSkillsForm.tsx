...
export function SystemSkillsForm() {
	...
	useEffect(() => {
		...
		const isDuplicate = systems.some(
			(s: { programName: string }) => s.programName.toLowerCase() === trimmedValue.toLowerCase(),
		);
	...
	return (
		<div className="mt-8">
			<h1 className="text-text-title font-bold text-3xl mb-6">Conocimientos en Sistemas</h1>
					...
					<div className="flex flex-col gap-4">
						...
						<div>
							...
							{errors.systems?.message && (
								<p className="text-red-500 text-sm mt-1">{errors.systems.message as string}</p>
							)}
						</div>
						...
						{systems.length === 0 && <p className="text-gray-500 text-sm">No has agregado ningún programa todavía.</p>}
					</div>
				)}
			/>
		</div>
