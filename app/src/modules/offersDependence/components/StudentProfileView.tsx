...
export default function StudentProfileView() {
    ...
    const fetchProfile = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`${API_BASE_URL}/offers-dependence/applicants/${id}/profile`, authOptions);
        ...
        <div className="space-y-6 mt-6 pr-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            <Badge variant="secondary" className="text-md">{profile.planName ?? "Programa no registrado"} -- {profile.planCode ?? "Código no registrado"}</Badge>
                            <Badge variant="secondary" className="text-md">Semestre: {profile.semester ?? "Semestre no registrado"}</Badge>
                        </div>
                        <p className="text-sm text-slate-500 mt-1">{profile.studentCode ? `Código: ${profile.studentCode}` : null}</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button variant="outline" size="sm" onClick={handleBack}>← Volver</Button>
                </div>
            </div>
                            ...
                            <Card>
                                ...
                                <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-md text-slate-700">
                                    <div className="space-y-3">
                                        <div><strong>Nombre:</strong> <EmptyHint value={profile.fullName} /></div>
                                        <div><strong>Código:</strong> <EmptyHint value={profile.studentCode} /></div>
                                        <div><strong>Identificación:</strong> <EmptyHint value={profile.idNumber} /></div>
                                        <div><strong>Correo electrónico:</strong> <EmptyHint value={profile.email} /></div>
                                        <div><strong>Teléfono:</strong> <EmptyHint value={profile.phone} /></div>
                                        <div><strong>Dirección:</strong> <EmptyHint value={profile.address} /></div>
                                        <div><strong>Sexo:</strong> <EmptyHint value={profile.gender} /></div>
                                        <div><strong>Fecha de nacimiento:</strong> <EmptyHint value={formatDate(profile.birthDate)} /></div>
                                        <div><strong>Edad:</strong> <EmptyHint value={profile.age ? `${profile.age} años` : undefined} /></div>
                                    </div>

                                    <div className="space-y-3">
                                        <div><strong>Programa:</strong> <EmptyHint value={profile.planName} /></div>
                                        <div><strong>Código del programa:</strong> <EmptyHint value={profile.planCode} /></div>
                                        <div><strong>Semestre:</strong> <EmptyHint value={profile.semester} /></div>
                                        <div><strong>Sede:</strong> <EmptyHint value={profile.campus} /></div>
                                        <div><strong>Periodo académico:</strong> <EmptyHint value={profile.academicPeriod} /></div>
                                        <div><strong>Jornada:</strong> <EmptyHint value={profile.jornada} /></div>
                                        <div><strong>Contacto de emergencia:</strong> <EmptyHint value={profile.emergencyContact} /></div>
                                        <div><strong>Teléfono de emergencia:</strong> <EmptyHint value={profile.emergencyPhone} /></div>
                                    </div>
                                </CardContent>
                            </Card>
                                    ...
                                    <div key={edu.id} className="border border-slate-200 bg-white rounded-xl p-4 shadow-sm">
                                        <div className="flex justify-between items-start gap-4">
                                            <div className="min-w-0">
                                                <p className="font-semibold text-slate-800 text-md truncate">Titulo obtenido: {edu.degreeTitle ?? "Título no registrado"}</p>
                                                <p className="text-md text-slate-600 mt-1">Institución: {edu.institution ?? "Institución no registrada"}</p>
                                                <p className="text-md text-slate-500 mt-1">Ciudad: {edu.city ?? "Ciudad no registrada"} {edu.endYear ? `-- Año finalización: ${edu.endYear}` : "Año no registrado"}</p>
                                            </div>
                                            <div className="font-semibold text-right text-md text-slate-800 space-y-1">
                                                <div>{edu.level ? educationLevel[edu.level] ?? "Nivel académico no registrado" : "Nivel académico no registrado"}</div>
                            ...
                            {profile.trainings && profile.trainings.length > 0 ? (
                                profile.trainings.map((t) => (
                                    <div key={t.id} className="border border-slate-200 bg-white rounded-xl p-4 shadow-sm">
                                        <div className="flex justify-between items-start gap-4">
                                            <div className="min-w-0">
                                                <p className="font-semibold text-slate-800 text-md truncate">Nombre de la capacitación: {t.courseName ?? "Curso no registrado"}</p>
                                                <p className="text-md text-slate-600 mt-1">Institución: {t.institution ?? "Institución no registrada"}</p>
                                                <p className="text-md text-slate-500 mt-1">Duración: {t.duration ?? ""} {t.endDate ? `-- Fecha finalización: ${formatDate(t.endDate)}` : "Fecha no registrada"}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))
                            ...
                            {profile.languages && profile.languages.length > 0 ? (
                                <div className="flex flex-wrap gap-3">
                                    {profile.languages.map((l) => (
                                        ...
                                        >
                                            <p className="font-semibold text-slate-800 text-md mb-2">
                                                Idioma: {l.language}
                                            </p>
                            ...
                            {profile.workExperiences && profile.workExperiences.length > 0 ? (
                                profile.workExperiences.map((exp) => (
                                    <div key={exp.id} className="border border-slate-200 bg-white rounded-xl p-4 shadow-sm">
                                        <div className="flex justify-between items-start gap-4">
                                            <div className="min-w-0">
                                                <p className="font-semibold text-slate-800 text-md truncate">Cargo: {exp.role ?? "Cargo no registrado"}</p>
                                                <p className="text-md text-slate-600 mt-1">Nombre empresa: {exp.companyName ?? "Empresa no registrada"}</p>
                                                <p className="text-md text-slate-500 mt-1">
                                                    Fecha: {exp.startDate ? formatDate(exp.startDate) : "No registrado"} — {exp.endDate ? formatDate(exp.endDate) : "Presente"}
                                                </p>
