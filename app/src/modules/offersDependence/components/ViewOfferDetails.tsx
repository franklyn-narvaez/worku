    ...
    const fetchOffer = useCallback(async () => {
        try {
            ...
            const response = await fetch(
                `${API_BASE_URL}/offers-dependence/${id}/details`,
                authOptions
            );
            if (!response.ok) {
            ...
            <Card className="border border-slate-200 shadow-sm">
                <CardHeader>
                    <div className="flex justify-between items-center">
                        ...
                        <Badge
                            variant={offer.status ? "success" : "destructive"}
                    ...
                    <div className="mt-4 pt-3 border-t">
                        ...
                        {offer.Application && offer.Application.length > 0 ? (
                            <div className="space-y-3">
                                {offer.Application.map((app) => {
                                    ...
                                    return (
                                        <div
                                            key={app.id}
                                            className="border border-slate-200 bg-white shadow-md rounded-xl p-4"
                                        >
                                            <div className="grid grid-cols-5 items-center gap-2 text-sm w-full">
                                                ...
                                                <p className="flex items-center gap-1 text-slate-500 text-sm truncate">
                                                    ✉️ {app.user.email}
                                                </p>

                                                <Badge
                                                    className={`${statusInfo.color} text-white justify-self-end`}
                                                >
                                                    {statusInfo.label}
                                                </Badge>
                                            </div>
                            ...
                            <p className="text-sm text-slate-500 mt-2">
                                No hay aplicantes registrados aún.
                            </p>
                        )}
